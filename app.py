...
TELEGRAM_BOT_TOKEN = os.environ.get('HOOKSHOT_TELEGRAM_TOKEN', '')

AUTO_DELIVER_KEY    = os.environ.get('HOOKSHOT_AUTO_KEY', '')

TG_USERS_FILE   = os.path.join(ROOT, 'telegram_users.json')

TG_PENDING_FILE = os.path.join(ROOT, 'telegram_pending.json')
...
def _auto_log(data):
    ...
    try:
        ...
        entry = json.dumps({'ts': _dt.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ'), **data})
...
def _tg_send_video(chat_id, file_path, caption='', thread_id=None):
    ...
    result = _tg_api('sendVideo', data=data, files={'video': (fname, video_data)})
...
def _run_auto_deliver_account(account_id, account_name, model_id, model_name, tg_chat_id, tg_topic_id):
    ...
    TEXT_STYLE = 'clean'   # Inter, no background bar
    ...
    try:
        ...
        # Captions for this model — include times_used for rotation ordering
        ...
        if model_name:
            ...
            cur.execute("SELECT id, caption, COALESCE(times_used,0) FROM captions WHERE COALESCE(active,1)=1")

        captions_raw = cur.fetchall()   # [(id, text, times_used), ...]



        # Videos for this model — include times_used for rotation ordering
        ...
        if model_id:
            ...
            cur.execute("SELECT id, COALESCE(times_used,0) FROM videos WHERE COALESCE(active,1)=1")
        ...
        if not captions_raw or not video_rows:
            ...
            _auto_log({"account": account_name, "skipped": "no_videos_or_captions"})

            return



        # Pre-probe all available videos once so the loop can't be killed mid-batch by a bad file
        ...
        # Sort by times_used ascending so least-used come first
        ...
        # Working pools for this batch — pop() guarantees no repeats within one batch.

        # If fewer items than BATCH_SIZE exist, refill from the sorted list mid-batch.

        cap_pool = list(captions_sorted)

        vid_pool = list(videos_sorted)
        ...
        label = f"Auto {datetime.now().strftime('%Y-%m-%d')} — {account_name}"
        ...
        for i in range(BATCH_SIZE):
            ...
            try:
                ...
                # Refill pools only if exhausted (edge case: pool smaller than BATCH_SIZE)

                if not cap_pool: cap_pool = list(captions_sorted)

                if not vid_pool: vid_pool = list(videos_sorted)

                # Pop from front — always the current least-used, never repeated this batch

                cap_id, caption_text, _ = cap_pool.pop(0)

                video_id, _             = vid_pool.pop(0)
                ...
                if not render_ok:
                    ...
                    _auto_log({'account': account_name, 'video': video_id, 'error': 'render_failed'})
                ...
                _auto_log({'account': account_name, 'i': i, 'exception': str(e)})

                continue

    except Exception as e:

        _auto_log({'account': account_name, 'fatal': str(e)})
...
@app.route('/api/auto-deliver', methods=['POST'])

def api_auto_deliver():

    """Internal endpoint: generate and send daily batches for all configured accounts."""
    ...
    cur.execute("""
        ...
        SELECT a.id, a.name, a.model_id, m.name, a.tg_chat_id, a.tg_topic_id
    ...
    if _auto_deliver_running.is_set():

        return jsonify(status='already_running'), 200



    launched = [row[1] for row in accounts]



    def _run_all_serial(accounts):

        _auto_deliver_running.set()

        try:

            for row in accounts:

                acc_id, acc_name, model_id, model_name, tg_chat_id, tg_topic_id = row

                _run_auto_deliver_account(acc_id, acc_name, model_id, model_name, tg_chat_id, tg_topic_id)

        finally:

            _auto_deliver_running.clear()



    t = threading.Thread(target=_run_all_serial, args=(accounts,), daemon=True)
...
def telegram_webhook():
    ...
    if text.startswith('/start'):
        ...
        pending = _tg_load(TG_PENDING_FILE)

        now = time.time()

        pending = {k: v for k, v in pending.items() if now - v.get('ts', 0) < 600}

        if code and code in pending:

            username = pending[code]['username']

            users = _tg_load(TG_USERS_FILE)

            users[username] = chat_id

            _tg_save(TG_USERS_FILE, users)

            del pending[code]

            _tg_save(TG_PENDING_FILE, pending)

            _tg_api('sendMessage', {'chat_id': chat_id, 'text': f'\u2705 Connected! Hi {username} \u2014 your approved HookShot videos will be sent here automatically.'})

        else:

            _tg_api('sendMessage', {'chat_id': chat_id, 'text': 'Code not found or expired. Go back to HookShot and click Connect Telegram to get a fresh code.'})
...
@app.route('/api/telegram/connect')
...
def telegram_connect():
    ...
    pending = {k: v for k, v in pending.items() if v.get('username') != username}

    pending[code] = {'username': username, 'ts': time.time()}

    _tg_save(TG_PENDING_FILE, pending)

    me = _tg_api('getMe')

    bot_name = (me or {}).get('result', {}).get('username', '')

    return jsonify(code=code, bot_username=bot_name, connected=False)
...
def telegram_status():
    ...
    return jsonify(connected=username in users)
...
def _tg_send_batch_background(chat_id, batch_dir, approved_files):

    """Run in background thread: send videos to Telegram. Does not block the request."""
    ...
    for fname in approved_files:
        ...
        if os.path.isfile(fpath):
            ...
            try:
                ...
                _tg_send_video(chat_id, fpath)

            except Exception:

                pass  # Logged by _tg_send_video / _tg_api



@app.route('/api/telegram/send-batch', methods=['POST'])

@login_required

def telegram_send_batch():
    ...
    try:
        ...
        if not os.path.isdir(batch_dir):
            ...
            return jsonify(error=f'Batch folder not found: {batch_id}'), 404

        if not approved_files:

            return jsonify(error='No files to send'), 400

        # Spawn background thread and return immediately

        t = threading.Thread(

            target=_tg_send_batch_background,

            args=(chat_id, batch_dir, approved_files),
        ...
        return jsonify(
            ...
            message='Videos are being sent in the background. Check Telegram shortly.',
        ...
        return jsonify(error='Server error: ' + str(e)[:250])
